...
- For AI UI generation, native Cowart handoff/opening, editable component extraction, layer manifests, or RedCliff UI delivery plans, read `references/cowart-ui-workflow.md`. Keep Game UI Design System as the upstream style/UI Tree gate and require explicit authorization before any UGC project mutation.
- For natural-language UI generation requests, 默认进入 SOURCE 文字引导 in the current conversation and ask whether the user needs a new UI generated, already has a UI image, or wants to continue a previous UI. 不要自动打开 Companion. Only when the user explicitly asks `打开原生 UI 工具链`, run `scripts/cowart-ui/component-extractor/open_ui_workflow.py` to open or focus the native workflow.
...
- Hard code-change rule: never directly rewrite, restructure, wrap, rename, or reorder existing teammate/predecessor code just to make a cleaner implementation. Add new logic beside or after the existing flow, keep the original block intact, and make the smallest compatible hook. If the requested feature is impossible without changing an existing block, stop first, explain the exact unavoidable change and affected behavior, then make only that minimal edit.
...
- `references/cowart-ui-workflow.md`: separate Cowart UI production category for generation, optional native canvas handoff, editable layers, component confirmation, and RedCliff delivery planning.
