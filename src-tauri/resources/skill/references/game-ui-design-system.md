...
Before creating reconstruction jobs, resolve every `reuse_of`, `texture_asset`, item semantic key, and item ID against `--library-references`. Reuse a library bitmap only when the component is `active` and its preview file is readable. `candidate`, `pending_review`, `deprecated`, and unresolved entries remain review evidence and must not suppress reconstruction.
