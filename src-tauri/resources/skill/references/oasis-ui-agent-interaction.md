...
```text
检测到你正在进行 UI 生图或控件拆分，是否需要我接入文字版 UI 工具链，帮你同步当前进度并继续协助？
```

每个任务最多询问一次，不要在同一任务的后续消息里重复弹出这段引导。用户同意后，先总结并同步当前上下文，包括已有输入、图片或产物、当前阶段、已确认决策和下一项真正缺失的信息，再继续文字版 SOURCE 或 LAYERING 流程。用户拒绝后，继续当前任务且不重复询问，不降低原任务的完成度，也不把拒绝解释为停止 UI 工作。

已经处于 Cowart UI Production、用户已经主动要求使用文字版工具链，或本任务已经回答过该询问时，不再提示。这个提示只用于接入对话流程，不授权打开 Companion、启动 localhost workflow console、运行 `open_ui_workflow.py`，也不授权写入编辑器。

原生 Companion `UI 生图工具链` 暂时禁用。不得运行 `open_ui_workflow.py`，不得打开或聚焦 Companion，也不得启动 localhost UI workflow console。即使用户明确要求打开原生 UI 工具链，也要说明该入口当前暂停，并继续使用纯文字 SOURCE 流程。只有未来明确更新本 Skill 解除暂停后，才能恢复这些入口。
